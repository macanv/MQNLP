...
from src.segment.generate_dict.probability import entropyOfList
from src.segment.generate_dict.sequence import genSubparts, genSubstr
    ...
    def computeAggregation(self, words_dict):
        ...
        if len(parts) > 0:
            self.aggregation = min(map(
                lambda p1_p2: self.freq/words_dict[p1_p2[0]].freq/words_dict[p1_p2[1]].freq,
                parts
            ))
