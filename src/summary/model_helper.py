...
from src.summary.utils import iterator_utils
from src.summary.utils import misc_utils as utils
from src.summary.utils import vocab_utils
...
def _single_cell(unit_type, num_units, forget_bias, dropout, mode,
    ...
    # dropout只用在train 过程，在后面使用dropout=1-dropout 这样可以保证在infer和dev的过程中dropout=1
    ...
    # Residual ResNet 残差网络，可以减少计算量，同时支持深度网络训练
...
def create_rnn_cell(unit_type, num_units, num_layers, num_residual_layers,
    ...
    Args:
      unit_type: string representing the unit type, i.e. "lstm". RNN 的类型
      num_units: the depth of each unit. RNN cell 的隐含层数
      num_layers: number of cells. RNN网络的层数
      num_residual_layers: Number of residual layers from top to bottom. For
        example, if `num_layers=4` and `num_residual_layers=2`, the last 2 RNN
        cells in the returned list will be wrapped with `ResidualWrapper`. 最后num_residual_layers 是残差网络层
