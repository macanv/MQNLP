...
from models.tf_impl.cnn_rnn_text_class.cnn_rnn import TextCNNRNN
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			def real_len(batches):
				return [np.ceil(np.argmin(batch + [0]) * 1.0 / FLAGS.max_pooling_size) for batch in batches]
			...
			def train_step(x_batch, y_batch):
				feed_dict = {
					...
					cnn_rnn.pad: np.zeros([len(x_batch), 1, FLAGS.embedding_sie, 1]),
			...
			train_batches = data_helper.batch_iter([x_train, y_train], FLAGS.batch_size, FLAGS.num_epochs)
